...
for jj in range(len(model_information)):
    ...
    afb=[]
    aft=[]
    for tt in np.arange(0.0,2.5,0.01): # time shift = 0.01
        time_window1=tt
    ...
    if len(tt1[(anb>np.mean(anb)+np.std(anb))])==0:
        print(model)
        continue
    time_window1=np.max(tt1[(anb>np.mean(anb)+np.std(anb))])+0.05 # minimum time + 0.5 
    time_window2=np.max(tt2[(anb<np.mean(anb)+np.std(anb))])-0.05 # maximum time - 0.5
...
if save_data:
    ...
    n6.to_csv(path+'model_list'+'.csv',index=False)