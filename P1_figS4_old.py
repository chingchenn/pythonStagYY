...
# 
fig,(aa1,aa2,aa3) = plt.subplots(3,2,figsize=(18,21))
ax2=aa1[1]
axx4=aa2[1]
axftop2=aa3[1]
...
for i, model in enumerate(model_list):
    ...
    zbot_conv = ma.array(data.Ftop, mask = mask_conv)
    if i < 3:
        axftop2.plot(x,zbot_conv,color=colors[i],label=label_list[i],lw=3)
    else:
        axftop2.plot(x,zbot_conv,color=colors[i],label=label_list[i],lw=1)
#  ------------------------------ figure setting ------------------------------
ax.set_ylim(20,0)
ax2.set_ylim(20,0)
...
for aa in [ax,ax3,ax2,axx4,axftop,axftop2]:
    aa.minorticks_on()
...
# fig.savefig('/Users/chingchen/Desktop/StagYY_Works/paper_europa_ice_shell/figureS4_v4.pdf')