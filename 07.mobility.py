...
if fig_mobility:
    ...
    for kk, model in enumerate(model_list):
        ...
        for uu in range(1,end):
            ...
            #vrms[uu-1] = ffsnap.vrms[0]
            average_vrms[uu-1] = np.average(ffsnap.vrms)
            vrms[uu-1] = np.average(ffsnap.vrms)
            snaptime[uu-1] = uu/1000
        #ax4.plot(snaptime,vsurf/vrms,color = '#849DAB',lw=5)
        ax4.plot(snaptime,vsurf/vrms,color = newcolors[kk],lw=5)
        
    #ax4.set_xlim(0-.01,0.5)
    ax4.set_ylim(0.0,0.5)
    #for qq in range(len(time)):
        #math.floor(time[qq]*1000)/1000
        
    #ax4.plot(ff.time, ff.Vrms,color = newcolors[kk],lw=5,label = 'time.dat')
    #ax4.plot(snaptime[snaptime>0],average_vrms[snaptime>0],color = newcolors[kk+1],lw=5,label = 'rprof.dat')
    ...
    #ax4.set_ylabel('Vrms',fontsize = labelsize)
